...
*/
					if (startswith("histogramme",input)){//histogramme filename
						...
						if (nbOfArgTab!=3){
							...
							write(sock,end,sizeof(char)*strlen(end));
							char * recv_image_name="../client_recv_image_from_server/histo.tga";
							receive_image(sock, recv_image_name);	
...
*/
						if (startswith("classifier",input)){//classifier+path+size+size(nb)
							...
							if (nbOfArgTab<3){
								...
								if (len==-1){//len==-1 c'est-a-dire 2 arg incorrect
									printf ("please check your 2 argument, it must be size of color");
									continue;
								}
								for (i=0;i<len;i++){
